...
{
    public partial class FRM_RepPriceOffer : Form
    {
        ...
        void display()
        {
            try
            {
                ...
                "Discount as 'الحسم على الفاتورة',Seller_N as 'اسم البائع',S_AmountPay as 'المبلغ المدفوع',S_AmountRem as 'المبلغ المتبقي'" +
                    " from Sell_BillOrder order by S_Bill_ID desc", con);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (cmbCusName.Text.Equals(""))
                {
                    ...
                    " as 'قيمة اجمالي الفاتورة',Description as 'وصف الفاتورة',Cus_Name as 'اسم العميل'," +
                    "Discount as 'الحسم على الفاتورة',Seller_N as 'اسم البائع',S_AmountPay as 'المبلغ المدفوع',S_AmountRem as 'المبلغ المتبقي'" +
                        " from Sell_BillOrder where Date_B_Bill between '" + dtpFrom.Value.Date.ToShortDateString() + "' and '" + dtpTo.Value.Date.ToShortDateString() + "' order by S_Bill_ID desc ", con);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                    dgvSellBills.DataSource = dt;
                }
                else
                {
                    ...
                    " as 'قيمة اجمالي الفاتورة',Description as 'وصف الفاتورة',Cus_Name as 'اسم العميل'," +
                    "Discount as 'الحسم على الفاتورة',Seller_N as 'اسم البائع',S_AmountPay as 'المبلغ المدفوع',S_AmountRem as 'المبلغ المتبقي'" +
                        " from Sell_BillOrder where (Date_B_Bill >= '" + dtpFrom.Value.Date.ToShortDateString() + "') and (Date_B_Bill <= '" + dtpTo.Value.Date.ToShortDateString() + "')" +
        ...
        {
            FRM_RepPriceOfferDet rsd = new FRM_RepPriceOfferDet(dgvSellBills.CurrentRow.Cells[0].Value.ToString());
            rsd.ShowDialog();
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            //    OleDbDataAdapter da = new OleDbDataAdapter("select * from S_D_BillOrder where S_Bill_ID='" + dgvSellBills.CurrentRow.Cells[0].Value + "'", con);
            //    da.Fill(pr.StoreDataSet.S_D_BillOrder);
            //      or
            //      datatable dt=new datatable();
            //      da.fill(dt);
            //    pr.s_B_billbinding.datasource=dt;
