...
{
    public partial class FRM_REPUintCurrent : Form
    {
        ...
        public FRM_REPUintCurrent()
        {
            ...
            try
            {
                OleDbDataAdapter da = new OleDbDataAdapter("select ID as 'رقم العملية', U_Number as 'الرقم',U_Name as 'الاسم',U_Date as 'تاريخ العملية',U_Seller as 'اسم البائع',U_Value as 'القيمة المحولة',U_Price as 'السعر',U_Type as 'الشركة' from UnitCurrent", con);
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                OleDbDataAdapter da = new OleDbDataAdapter("select ID as 'رقم العملية',U_Number as 'الرقم',U_Name as 'الاسم',U_Date as" +
                    ...
                    "'السعر',U_Type as 'الشركة' from UnitCurrent where U_Number+U_Name+U_Date like '%" + txtSearch.Text + "%'", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                dgvUnit.DataSource = dt;
            }
        ...
        private void btnCalc_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    OleDbDataAdapter da = new OleDbDataAdapter("select sum(U_Price),sum(U_Value) from UnitCurrent", con);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
        ...
        private void btnPull_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    OleDbDataAdapter da = new OleDbDataAdapter("select U_Number,U_Name,U_Date,U_Value,U_Price,U_Rem,U_Seller,U_Type from UnitCurrent", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                ...
                con.Open();
                OleDbCommand cmd1 = new OleDbCommand("delete from UnitCurrent", con);
                ...
                try
                {
                    OleDbDataAdapter da2 = new OleDbDataAdapter("select U_Number as 'الرقم',U_Name as 'الاسم',U_Date as 'تاريخ العملية',U_Seller as 'اسم البائع',U_Value as 'القيمة المحولة',U_Price as 'السعر',U_Type as 'الشركة' from UnitCurrent", con);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        con.Open();
                        OleDbCommand cmd = new OleDbCommand("delete from UnitCurrent where ID = " + dgvUnit.CurrentRow.Cells[0].Value.ToString() + "", con);
                        cmd.ExecuteNonQuery();
                        con.Close();
